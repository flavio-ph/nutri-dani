...
export default function FoodAutocomplete({ item, placeholder = 'Buscar alimento…', onChange }: Props) {
  ...
  function handleQueryChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (selectedFood) {
      ...
      onChange({ ...item, foodId: undefined, food: e.target.value, quantity: '', calories: undefined, proteins: undefined, carbs: undefined, lipids: undefined });
    }
  }
  ...
  function handleClear() {
    ...
    setMeasureIdx(0);
    setQty(1);
    onChange({ ...item, food: '', quantity: '', foodId: undefined, measureType: undefined, measureWeight: undefined, measureQty: undefined, calories: undefined, proteins: undefined, carbs: undefined, lipids: undefined });
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            ...
            <input
              ...
              className="w-16 text-sm text-center bg-[#F4F7F2] dark:bg-[#1A221A] text-[#2C2C2C] dark:text-[#FDFBF7] border border-[#E1E8DE] dark:border-[#2A3526] rounded-lg px-2 py-1 focus:outline-none focus:ring-1 focus:ring-[#5E7153]"
            />
          </div>

          {/* Medida caseira */}
          <div className="flex items-center gap-1 flex-1 min-w-[140px]">
            <label htmlFor={`${uid}-measure`} className="text-[10px] text-[#5E7153] font-semibold uppercase tracking-wide whitespace-nowrap">
              Medida
            </label>
            <div className="relative flex-1">
              <select
                ...
                className="w-full appearance-none text-xs bg-[#F4F7F2] dark:bg-[#1A221A] text-[#2C2C2C] dark:text-[#FDFBF7] border border-[#E1E8DE] dark:border-[#2A3526] rounded-lg pl-2 pr-6 py-1 focus:outline-none focus:ring-1 focus:ring-[#5E7153] cursor-pointer"
              >
                {STANDARD_MEASURES.map((m, i) => (
                  <option key={m.label} value={i}>{m.label}</option>
                ))}
              </select>
              <ChevronDown size={11} className="pointer-events-none absolute right-1.5 top-1/2 -translate-y-1/2 text-[#5E7153]" />
            </div>
          </div>
